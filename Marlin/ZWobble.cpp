...
#if ENABLED(Z_WOBBLE_COMPENSATION_SERIAL_LOGGING)
...
long ZWobble::InsertCorrection(const float &targetZ, int32_t position[NUM_AXIS])
{
  
  if (!m_consistent) return 0; // don't go through consistency checks all the time; just check one bool
  
  float originZ = (float)position[Z_AXIS]/planner.axis_steps_per_mm[Z_AXIS];
  ...
  long stepDiff = lround((targetZRod-originZRod)*planner.axis_steps_per_mm[Z_AXIS]) - (lround(targetZ*planner.axis_steps_per_mm[Z_AXIS]) - position[Z_AXIS]);
